...
parameters = [ 0.05, 0.1, 0.25, 0.5, 0.75, 1, 1.5, 2, 5;... % threshold
    0.1, 0.25, 0.5, 0.75, 0.9, 1, 1.1, 1.5, 2;              % p2
    0.1, 0.25, 0.5, 0.75, 0.9, 1, 1.1, 1.5, 2];             % p3
outputs = [1]; % no output options in this model
out_type = 1;
...
parfor i = 1:length(parameter_set(:,1))
    ...
    F = @(y) NFL_fig1(0, y, parameter_set(i,:), 2);
    ...
    for k = 1:2*(time/T)
        ...
        [T_iter,Y_iter] = ode45(@(t, y) NFL_fig1(t, y, parameter_set(i,:), out_type), t_interval, x0);
        To = [To; T_iter];
        Y = [Y; Y_iter];
    end
    % interpret results
