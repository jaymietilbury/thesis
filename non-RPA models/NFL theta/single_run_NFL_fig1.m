...
out_type = 8; % no output options in this model - initialise with large number
...
response_cell = {'Responses for NFL almost perfect', 0,0,0,0,0,0};
...
F = @(y) NFL_fig1(0, y, parameter_set(1,:), 2);
...
for i = 1:2*(time/T)
    ...
    [T_iter,Y_iter] = ode45(@(t, y) NFL_fig1(t, y, parameter_set(1,:), out_type), t_interval, x0);
    To = [To; T_iter];
    Y = [Y; Y_iter];
end
...
% calculate vector with length T to plot threshold
...
title(['NFL figure 1 oscillatory: ' 'period = ', num2str(parameter_set(1,2)), ', duration = ', num2str(parameter_set(1,3)), ', I0 = ', num2str(parameter_set(1,4)), ', lambda = ', num2str(parameter_set(1,5)), ', kappa = ', num2str(parameter_set(1,6))], 'Fontsize', 12)
