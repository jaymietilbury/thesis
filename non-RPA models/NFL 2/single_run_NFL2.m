...
response_cell = {'Responses for NFL 2', 0,0,0,0,0,0,0};

% fsolve for steady state
x0 = ones(nodes,1);
options = optimoptions('fsolve', 'Display', 'off');
F = @(y) NFL_2(0, y, parameter_set(1,:), out_type, 0);
...
for i = 1:2*(time/T)
    ...
    [T_iter,Y_iter] = ode45(@(t, y) NFL_2(t, y, parameter_set(1,:), out_type, 1), [t_start, t_end], x0);
    To = [To; T_iter];
    Y = [Y; Y_iter];
end
...
legend('Input', 'Output', 'R', 'I', 'I0 line out', 'Fontsize', 16, 'Location', 'northeastoutside');
title(['NFL 2', ', lambda = ', num2str(parameters(1)), ', I0 out = ', num2str(parameters(2)), ', n out = ', num2str(parameters(3)), ', lambda out = ', num2str(parameters(4))', ', out type = ', num2str(out_type)], 'Fontsize', 18)
