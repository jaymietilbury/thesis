                ...
                for p3 = 1:size(parameters,2)
                    for p4 = 1:size(parameters,2)
                        for out_type = 1:size(outputs,2)
                            if index == 1
                                parameter_set = [index, T(t), d(dur), parameters(1, p1), parameters(2, p2), parameters(3,p3), parameters(4, p4), outputs(out_type)];
                            else
                                parameter_set = [parameter_set; index, T(t), d(dur), parameters(1, p1), parameters(2, p2), parameters(3,p3), parameters(4, p4), outputs(out_type)];
                            end
                            index = index + 1;
...
response_cell = {'Responses for IFFL_2', 0,0,0,0,0,0,0};
...
parfor i = 1:length(parameter_set(:,1))
    ...
    F = @(y) IFFL_2(0, y, parameter_set(i,:), out_type, 0);
    ...
    for k = 1:2*(time/T)
        ...
        [T_iter,Y_iter] = ode45(@(t, y) IFFL_2(t, y, parameter_set(i,:), out_type, 1), [t_start, t_end], x0);
        To = [To; T_iter];
        Y = [Y; Y_iter];
    end
    % interpret results
    ...
    % calulcate input vector with same length as To vector
    In = To;
    for s = 1:length(To)
    ...
    row = {response, parameter_set(i,2), parameter_set(i,3), parameter_set(i,4), parameter_set(i,5), parameter_set(i,6), parameter_set(i,7), out_type};
